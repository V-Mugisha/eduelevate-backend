...
export async function createProfile(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!userId) { res.status(401).json({ message: "Auth required" }); return; }
...
export async function apply(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const app = await mentorshipService.applyForMentorship(userId, req.params.educatorId as string, data);
...
export async function sendMessage(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const msg = await mentorshipService.sendMessage(req.params.id as string, userId, content.trim());
