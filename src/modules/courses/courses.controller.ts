...
import { createCourseSchema, updateCourseSchema, courseQuerySchema } from "./courses.dto.js";
...
export async function createCourse(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const course = await coursesService.create(data, userId);
...
export async function updateCourse(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const course = await coursesService.update(req.params.id as string, data, userId);
    res.json({ data: course });
  } catch (error) {
    handleError(error, res, next);
  }
...
export async function deleteCourse(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await coursesService.remove(req.params.id as string, userId);
